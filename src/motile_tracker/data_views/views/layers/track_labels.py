...
def split_paint_atom(atom) -> list[tuple]:
    """Split a napari paint history atom into one segmentation update per label.

    There are two forms:
    - napari ≥ 0.8 records a bounding box plus a mask of the changed pixels inside it.
    - napari ≤ 0.7 uses ``data_setitem`` records a plain multi-index. Funtracks has to
      build the mask from the coordinates.

    Args:
        atom: a paint "atom" from the labels layer's undo history. Either a
            ``_MaskedPaintAtom`` (bounding box, mask of the changed pixels inside
            it, their values before the change, and the value painted), or a
            ``data_setitem`` 3-tuple (multi-index, values before the change, and
            the value after it).

    Returns:
        list[tuple]: one entry per label painted over in this atom, per time point,
        either as (mask, time, old value) or as (multi-index, old value).
    """

    if len(atom) == 3:  # a data_setitem atom, only has pixel coordinates
        indices, old_values, _new_value = atom
        old_values = np.broadcast_to(np.asarray(old_values), indices[0].shape)
        updates = []
        for old_value in np.unique(old_values):
            ...
            for time in np.unique(indices[0][of_value]):
                ...
                updates.append(
                    (tuple(axis[in_slice] for axis in indices), int(old_value))
                )
        return updates

    slice_key, mask, old_values, _new_value = atom
    if mask is None:
        # every pixel in the bounding box changed, so napari dropped the mask and
        # stored a snapshot of the whole box instead
        old_region = np.asarray(old_values)
        mask = np.ones(old_region.shape, dtype=bool)
    else:
        old_region = np.zeros(mask.shape, dtype=np.asarray(old_values).dtype)
        old_region[mask] = old_values

    # tracksdata bounding boxes are spatial only, so peel the time axis off the
    # bounding box and report it separately
    first_time = 0 if slice_key[0].start is None else slice_key[0].start
    offset = np.array([0 if sl.start is None else sl.start for sl in slice_key[1:]])

    # The atom's own bounding box is kept as is: funtracks combines the atoms of a
    # stroke, and tightens the box around each label once it has them all.
    bbox = np.concatenate([offset, offset + np.array(mask.shape[1:])])
    ...
    for step, (mask_t, old_t) in enumerate(zip(mask, old_region, strict=True)):
        for old_value in np.unique(old_t[mask_t]):
            ...
            updates.append(
                (Mask(changed, bbox=bbox), first_time + step, int(old_value))
            )
    return updates
    ...
    def _parse_paint_event(self, event_val):
        ...
        Args:
            event_val (list[tuple]): A list of paint "atoms" generated by the labels
                layer. See ``split_paint_atom`` for the shapes an atom can have.

        Returns:
            tuple(int, list[tuple]): The new value, and the updates for every label
                painted over, as ``split_paint_atom`` reports them. A label painted
                over by several atoms of a stroke appears once per atom;
                ``UserUpdateSegmentation`` combines those.
        """

        new_value = event_val[-1][-1]
        return new_value, [
            update for atom in event_val for update in split_paint_atom(atom)
        ]
