...
*September 2026 - motile-tracker v5.\* - Caroline Malin-Mayor - Teun Huijben - Anniek Stokkermans*

[`Napari Track Edit (Motile Tracker)`](https://www.Napari-hub.org/plugins/motile-tracker) is a Napari plugin for interactive visualization, navigation, and editing of object tracking results. It uses [`motile`](https://funkelab.github.io/motile/) for cell tracking with optimization, but you can also manually track from scratch or open existing tracking data.
...
<div style="display: flex; flex-wrap: nowrap; justify-content: center; align-items: flex-start; gap: 20px; margin: 20px 0; break-inside: avoid; page-break-inside: avoid;">
    <figure style="flex: 0 1 auto; width: 430px; min-width: 0; margin: 0; text-align: center; break-inside: avoid; page-break-inside: avoid;">
        ...
        <figcaption style="margin-top: 6px; font-size: 0.9em;">Motile Tracker startup screen</figcaption>
    </figure>
</div>

### Plugin layout
This tutorial will walk you through the main functionalities on the different plugin widgets.
All plugin widgets are listed under `Plugins > Motile Tracker`, where they can be (re)opened individually or all at once.
...
To download and open the sample data, click `File > Open Sample > Motile Tracker > Fluo-N2DL-HeLa (2D)` and `File > Open Sample > Motile Tracker > Mouse Embryo Membranes (3D)`.
...
## Generating Tracks
There are two ways to obtain a tracking result: manual tracking on an image layer, or automatic tracking using Motile on a Labels or Points layer. We will explore both methods here.

### Manual tracking
Manual tracking from scratch requires that you open an `Image` layer first. In the `Tracks List` widget you can select this layer, and choose to either `Track with Points` or `Track with Labels`. Depending on your choice, this will create either an empty `Points` or empty `Labels` layer, where you can manually add points or paint labels to track objects.
...
<div style="display: flex; flex-wrap: nowrap; align-items: flex-start; gap: 20px; margin: 20px 0; break-inside: avoid; page-break-inside: avoid;">
    <div style="flex: 1 1 auto; min-width: 0;">
        To track objects with Motile, you need to either provide object segmentations on a Napari <code>Labels</code> layer or object detections on a <code>Points</code> layer. Tracking parameters should be specified in the <code>Tracking</code> tab and are subdivided in hyperparameters, constant costs and attribute weights.
...
<div style="background-color: #e6f7ff; border: 2px solid #104982; padding: 15px; border-radius: 10px; margin: 20px 0; break-inside: avoid; page-break-inside: avoid; -webkit-print-color-adjust: exact; print-color-adjust: exact;">
    ...
    <p>5. Click <code>Back to editing</code> and test multiple combinations of parameters. Note that you can also use the Points layer 'centroids' as input. Compare the different tracking results. Verify that you can find previous results in the <code>Tracks List</code> tab.</p>
</div>

## Viewing and navigating a tracking result

### Track visualization
Tracking results are typically displayed with:
- a **Points** layer, with nodes color-coded by tracklet ID and with symbols matching the state of the node:
  - △ dividing node
  - ✕ end point node
  - ◯ linear node
- an optional **Segmentation** (Labels) layer (only if you provided a Labels layer as input), with label values matching node ids, but color-coded by tracklet ID
...
- a **Table View** with all object properties is optionally visible in the `Table` widget.
...
<div style="background-color: #e6f7ff; border: 2px solid #104982; padding: 15px; border-radius: 10px; margin: 20px 0; break-inside: avoid; page-break-inside: avoid; -webkit-print-color-adjust: exact; print-color-adjust: exact;">
  <h2>Exercise 2 - View and explore a tracking result</h2>
  <p>1. Inspect your Motile tracking result in the Napari <code>Viewer</code>, the <code>Lineage View</code>, and in the <code>Table</code> widget. You can click in any of these views to select and center nodes and use scroll to zoom in to specific regions. Use right-mouse click to reset the Lineage View.</p>
  ...
  <p>3. Open the <code>Editing & Selection</code> widget and look at the 'Selection' section. Test the buttons and keyboard shortcuts to clear, restore, and cycle through selections.</p>
  <p>4. Hover over nodes in the <code>Lineage View</code> and inspect the information you can read there. What is the difference between a Track and a Lineage?</p>
  <p>5. Open the <code>Visualization</code> tab and switch the Display Mode to 'Lineage'. What happens to the labels? Try the different sliders for 'highlight', 'foreground', and 'background' opacity, do you understand what they each refer to? What happens if you set the 'contour' value to 1 in the layer controls (top left menu)?</p>
  <p>6. Open the <code>Lineage View</code> controls and set the display mode to 'Current lineage(s)', and change your selection by clicking on different nodes in the <code>Viewer</code>. Try out the arrow buttons or arrow keys to move up or down the tree or to jump to neighboring nodes. </p>
  <p>7. If you have time, close all layers and open <code>`File > Open Sample > Motile Tracker > Mouse Embryo Membranes (3D)`</code>. Go back to the <code>Tracking</code> widget to select settings (deactivate the IoU attribute to speed up computation) and run the tracking. Once your tracking result is ready, activate the Orthogonal Views in the <code>Visualization</code> widget and explore the data. Test the different syncing and visualization options in the OrthoView control widget and try out the <code>T</code> shortcut on the different (ortho) views. </p>
</div>
...
<div style="background-color: #e6f7ff; border: 2px solid #104982; padding: 15px; border-radius: 10px; margin: 20px 0; break-inside: avoid; page-break-inside: avoid; -webkit-print-color-adjust: exact; print-color-adjust: exact;">
  ...
  <p>5. Select two connected nodes and use the <code>Connect/Disconnect</code> button or press <code>C</code> to break the connection. What happens to the two fragments?</p>
  <p>6. Select two nodes and try to create an edge between them with the <code>Connect/Disconnect</code> button or by pressing <code>C</code>. Try to answer these questions: </p>
    <ul style="padding-left: 60px;">
    ...
    <li> What happens if you connect a node that already has an outgoing edge to another node? </li>
...
<div style="display: flex; flex-wrap: nowrap; align-items: flex-start; gap: 20px; margin: 20px 0; break-inside: avoid; page-break-inside: avoid;">
    <div style="flex: 1 1 auto; min-width: 0;">
        You can save your tracking results in the <code>Tracks List</code> tab. This will save the parameters and the tracking data to the displayed destination. You can load your results back in at the bottom of the <code>Tracks List</code> tab by loading a Motile Run and selecting the folder. In addition, Motile Tracker allows you to import and export the results from/to <code>csv</code> and <code><a href="https://github.com/live-image-tracking-tools/geff">geff</code></a> via the dropdown menu at the bottom and the export (middle) button in the Results List.
    </div>
...
| `C` | Connect selected nodes (disconnect if connected already) |
